...
-(void)textFieldChanged:(NSNotification *)sender {
    ...
    if (txtField.text.length == 0){
        self.isSearching = NO;
        [self setQuestionScope:tempType];
    }
}
...
- (void)viewDidLoad
{
    ...
    tempType = CategoryType_MyQuestion;
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if ([title isEqualToString:@"修改回复"]) {
        ...
        QuestionModel * q_obj = (QuestionModel *)answer.questionModel;
        AnswerModel *a_obj = (AnswerModel *)[q_obj.answerList lastObject];
        
        RichContextObj *obj = (RichContextObj *)[a_obj.answerRichContentArray lastObject];
        content = [self returnAnsweContentWith:obj.richContext];
    }
    if ([title isEqualToString:@"修改回答"]) {
        editTip = @"修改回答";
        [CaiJinTongManager shared].reaskType = ReaskType_ModifyAnswer;
        
        RichContextObj *obj = (RichContextObj *)[answer.answerRichContentArray lastObject];
        content = [self returnAnsweContentWith:obj.richContext];
    }
    ...
    [self presentViewController:self.typeContentController animated:YES completion:^{
        ...
        self.typeContentController.inputTextView.text = content;
    }];
...
-(void)drTreeTableView:(DRTreeTableView *)treeView didSelectedTreeNode:(DRTreeNode *)selectedNote{
    ...
    switch ([selectedNote.noteRootContentID integerValue]) {
        case CategoryType_AllQuestion://所有问答
        {
            ...
            tempType = CategoryType_AllQuestion;
        ...
        case CategoryType_MyAnswer://我的回答
        {
            ...
            tempType = CategoryType_MyAnswer;
